...
in_path = os.path.join("/Users/jcachat/torchlight_ai/spruce_maplayer_dashboard/merge")
out_path = os.path.join("/Users/jcachat/torchlight_ai/spruce_maplayer_dashboard/merge/output/")

os.makedirs(in_path, exist_ok=True)
os.makedirs(out_path, exist_ok=True)

df = pd.read_parquet('/Users/jcachat/torchlight_ai/spruce_maplayer_dashboard/merge/input')

for i in range(10):
    dfcopy = df.copy()
    dfcopy["file"] = i
    dfcopy.to_parquet(os.path.join(in_path, f"temp_merge_{i}.parquet"))
    
coalese_parquets(in_path, outpath=os.path.join(out_path, "complete_merge.parquet"))
print(pq.ParquetFile(os.path.join(out_path, "complete_merge.parquet")).metadata)
